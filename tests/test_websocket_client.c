...
void test_ws_state_to_string_should_return_correct_strings(void) {
    TEST_ASSERT_EQUAL_STRING("DISCONNECTED", ws_state_to_string(WS_STATE_DISCONNECTED));
    TEST_ASSERT_EQUAL_STRING("CONNECTING", ws_state_to_string(WS_STATE_CONNECTING));
    TEST_ASSERT_EQUAL_STRING("CONNECTED", ws_state_to_string(WS_STATE_CONNECTED));
    TEST_ASSERT_EQUAL_STRING("DISCONNECTING", ws_state_to_string(WS_STATE_DISCONNECTING));
    TEST_ASSERT_EQUAL_STRING("ERROR", ws_state_to_string(WS_STATE_ERROR));
}

void test_ws_state_to_string_should_handle_invalid_state(void) {
    // Act
    const char *str = ws_state_to_string((ws_state_t)999);
...
void test_ws_error_to_string_should_return_correct_strings(void) {
    TEST_ASSERT_EQUAL_STRING("NO_ERROR", ws_error_to_string(WS_ERROR_NONE));
    TEST_ASSERT_EQUAL_STRING("CONNECTION_FAILED", ws_error_to_string(WS_ERROR_CONNECTION_FAILED));
    TEST_ASSERT_EQUAL_STRING("SEND_FAILED", ws_error_to_string(WS_ERROR_SEND_FAILED));
    TEST_ASSERT_EQUAL_STRING("TIMEOUT", ws_error_to_string(WS_ERROR_TIMEOUT));
}

void test_ws_error_to_string_should_handle_invalid_error(void) {
    // Act
    const char *str = ws_error_to_string((ws_error_t)999);
