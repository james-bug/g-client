...
static int connect_to_agent(void) {
    ...
    if (g_vpn_ctx.sockfd < 0) {
        #ifndef TESTING
        log_error("Failed to connect to VPN agent: %s", g_vpn_ctx.socket_path);
        #endif
        return -1;
    }
...
static int send_command(const char *action) {
    ...
    if (sent < 0) {
        #ifndef TESTING
        log_error("Failed to send VPN command: %s", action);
        #endif
        return -1;
    }
    
    #ifndef TESTING
    log_debug("VPN command sent: %s", action);
    #endif
...
static int receive_response(char *response, size_t max_len) {
    ...
    if (received < 0) {
        ...
        #ifndef TESTING
        log_error("Failed to receive VPN response");
        #endif
        return -1;
    }
    ...
    if (received == 0) {
        // Connection closed
        #ifndef TESTING
        log_warn("VPN agent closed connection");
        #endif
        return -1;
    }
    ...
    #ifndef TESTING
    log_debug("VPN response received: %s", response);
    #endif
...
int vpn_controller_init(const char *socket_path) {
    ...
    #ifndef TESTING
    log_info("VPN controller initialized: %s", g_vpn_ctx.socket_path);
    #endif
...
int vpn_controller_process(int timeout_ms) {
    ...
    if (is_timeout(g_vpn_ctx.operation_start_time, g_vpn_ctx.operation_timeout)) {
        #ifndef TESTING
        log_warn("VPN operation timeout");
        #endif
        ...
        if (should_retry()) {
            ...
            #ifndef TESTING
            log_info("VPN retry attempt %d/%d", g_vpn_ctx.retry_count, VPN_MAX_RETRY_ATTEMPTS);
            #endif
...
void vpn_controller_cleanup(void) {
    ...
    #ifndef TESTING
    log_info("VPN controller cleaned up");
    #endif
}
