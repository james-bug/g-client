...
static void change_state(client_context_t *ctx, client_state_t new_state) {
    ...
    #ifndef TESTING
    log_info("Client state changed: %s -> %s",
...
static void update_led_for_state(client_context_t *ctx, client_state_t state) {
    ...
    switch (state) {
        ...
        case CLIENT_STATE_VPN_CONNECTING:
            ...
            color.g = 255;
            color.b = 0;
            led_controller_set_color(color.r, color.g, color.b);
            led_controller_set_blink(500, 500);
            break;
            
        case CLIENT_STATE_VPN_CONNECTED:
        case CLIENT_STATE_WS_CONNECTING:
            // Yellow solid
            led_controller_set_color(255, 255, 0);
            led_controller_stop_blink();
            break;
            
        case CLIENT_STATE_QUERYING_PS5:
            // Blue blinking
            led_controller_set_color(0, 0, 255);
            led_controller_set_blink(250, 250);
            break;
            
        case CLIENT_STATE_ERROR:
            // Red blinking
            led_controller_set_color(255, 0, 0);
            led_controller_set_blink(200, 200);
            break;
...
static void update_led_for_ps5_status(client_context_t *ctx, ps5_status_t status) {
    ...
    switch (status) {
        case PS5_STATUS_ON:
            // Green
            led_controller_set_color(0, 255, 0);
            led_controller_stop_blink();
            break;
            
        case PS5_STATUS_STANDBY:
            // Orange
            led_controller_set_color(255, 165, 0);
            led_controller_stop_blink();
            break;
        ...
        default:
            // Purple (unknown)
            led_controller_set_color(128, 0, 128);
            led_controller_stop_blink();
            break;
    }
    
    log_info("LED updated for PS5 status: %s", ps5_status_to_string(status));
    #endif
}
...
static void on_button_event(button_event_t event, void *user_data) {
    ...
    #ifndef TESTING
    log_info("Button event: %s", button_event_to_string(event));
    #endif
...
static void on_vpn_state_change(vpn_state_t old_state, vpn_state_t new_state, void *user_data) {
    ...
    #ifndef TESTING
    log_info("VPN state changed: %s -> %s", 
             vpn_state_to_string(old_state), vpn_state_to_string(new_state));
    #endif
 ...
 */
static void on_ws_message(const char *message, void *user_data) {
    client_context_t *ctx = (client_context_t *)user_data;
    ...
    #ifndef TESTING
    log_info("WebSocket message received: %s", message);
    #endif
...
static void on_ws_error(ws_error_t error, const char *message, void *user_data) {
    ...
    #ifndef TESTING
    log_error("WebSocket error: %s - %s", ws_error_to_string(error), message);
    #endif
    
    report_error(ctx, CLIENT_ERROR_WS_FAILED, message);
}
...
static void handle_error_state(client_context_t *ctx) {
    ...
    if (ctx->error_count < ctx->config.max_retry_attempts && ctx->config.auto_retry) {
        ...
        usleep(CLIENT_RETRY_INTERVAL_S * 1000000);
        
        #ifndef TESTING
        log_info("Retrying after error (attempt %d/%d)", 
...
int client_sm_init(client_context_t *ctx) {
    ...
    if (button_handler_init(ctx->config.button_pin, ctx->config.button_debounce_ms) < 0) {
        log_error("Failed to initialize button handler");
        return -1;
    }
    ...
    if (vpn_controller_init(ctx->config.vpn_socket_path) < 0) {
        #ifndef TESTING
        log_error("Failed to initialize VPN controller");
    ...
    ws_client_set_callbacks(on_ws_connected, on_ws_disconnected, on_ws_error, on_ws_message, ctx);
    ...
    #ifndef TESTING
    log_info("Client state machine initialized");
    #endif
...
void client_sm_update(client_context_t *ctx) {
    ...
    #endif
    vpn_controller_process();
    ws_client_process(10);
...
client_state_t client_sm_get_state(client_context_t *ctx) {
    if (ctx == NULL) {
...
ps5_status_t client_sm_get_ps5_status(client_context_t *ctx) {
    if (ctx == NULL) {
...
void client_sm_get_stats(client_context_t *ctx, client_stats_t *stats) {
...
void client_sm_cleanup(client_context_t *ctx) {
    ...
    #ifndef TESTING
    log_info("Client state machine cleaned up");
    #endif
}
