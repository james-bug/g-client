...
static void change_state(ws_state_t new_state) {
    ...
    #ifndef TESTING
    log_info("WebSocket state changed: %s -> %s",
    ...
    if (new_state == WS_STATE_CONNECTED && g_ws_ctx.on_connected != NULL) {
        ...
        g_ws_ctx.on_disconnected(g_ws_ctx.user_data);
    } else if (new_state == WS_STATE_ERROR && g_ws_ctx.on_error != NULL) {
        g_ws_ctx.on_error(WS_ERROR_CONNECTION_FAILED, "Connection error", g_ws_ctx.user_data);
    }
}
        ...
        case LWS_CALLBACK_CLIENT_RECEIVE:
            if (len > 0 && len < WS_MAX_MESSAGE_SIZE) {
                ...
                if (g_ws_ctx.on_message != NULL) {
                    g_ws_ctx.on_message(g_ws_ctx.recv_buffer, g_ws_ctx.user_data);
                }
            }
            break;
...
int ws_client_init(const char *server_host, int server_port) {
    ...
    #ifndef TESTING
    log_info("WebSocket client initialized: %s:%d", server_host, server_port);
    #endif
...
int ws_client_send(const char *message) {
    ...
    log_debug("WebSocket message queued: %s", message);
    #endif
...
int ws_client_service(int timeout_ms) {
    ...
    if (g_ws_ctx.current_state == WS_STATE_DISCONNECTED ||
        ...
        if (should_reconnect()) {
            #ifndef TESTING
            log_info("Attempting WebSocket reconnection (attempt %d/%d)",
    ...
    if (g_ws_ctx.current_state == WS_STATE_CONNECTED) {
        ...
        if (g_ws_ctx.waiting_for_pong &&
            ...
            #ifndef TESTING
            log_warn("WebSocket pong timeout, disconnecting");
            #endif
...
void ws_client_cleanup(void) {
    ...
    #ifndef TESTING
    log_info("WebSocket client cleaned up");
    #endif
}
